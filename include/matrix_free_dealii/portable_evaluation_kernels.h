...
#ifndef evaluation_kernels_h
#define evaluation_kernels_h
...
namespace Copy
{
  namespace Portable
  {
    namespace internal
    {
       ...
       */
      constexpr bool
      use_collocation_evaluation(const unsigned int fe_degree, const unsigned int n_q_points_1d)
      {
      ...
      struct FEEvaluationImpl
      {
                 ...
                 const typename MatrixFree<dim, Number>::Data *data)
        {
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              auto u = Kokkos::subview(data->shared_data[dof_handler_index].values, Kokkos::ALL, c);
              auto grad_u = Kokkos::subview(data->shared_data[dof_handler_index].gradients,
                                            Kokkos::ALL,
                                            Kokkos::ALL,
                                            c);
              ...
              if constexpr (dim == 1)
              ...
              else if constexpr (dim == 3)
                {
                  constexpr int temp1_size = Utilities::pow(fe_degree + 1, 2) * n_q_points_1d,
                                temp2_size = Utilities::pow(n_q_points_1d, 2) * (fe_degree + 1);
              ...
              if constexpr (dim == 1)
              ...
              else if constexpr (dim == 3)
                {
                  constexpr int temp1_size = Utilities::pow(n_q_points_1d, 2) * (fe_degree + 1),
                                temp2_size = Utilities::pow(fe_degree + 1, 2) * n_q_points_1d;
        ...
        struct FEEvaluationImpl
        {
                   ...
                   const BatchedData<dim, Number>        *data)
          {
            ...
            for (unsigned int c = 0; c < n_components; ++c)
              {
                auto u =
                  Kokkos::subview(data->shared_data[dof_handler_index].values, Kokkos::ALL, c);
                auto grad_u = Kokkos::subview(data->shared_data[dof_handler_index].gradients,
                                              Kokkos::ALL,
                                              Kokkos::ALL,
                                              c);
            ...
            for (unsigned int c = 0; c < n_components; ++c)
              {
                ...
                if constexpr (dim == 1)
                  {
                    const auto &temp = data->shared_data[dof_handler_index].scratch_pad;
