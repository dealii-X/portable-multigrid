...
#ifndef portable_hanging_nodes_internal_h
#define portable_hanging_nodes_internal_h
  ...
  namespace Portable
  {
    namespace internal
    {
      ...
      is_constrained_dof_2d(
        const dealii::internal::MatrixFreeFunctions::ConstraintKinds &constraint_mask,
      ...
      {
        const unsigned int face1_idx = (direction == 0) ? y_idx : (direction == 1) ? z_idx : x_idx;
        const unsigned int face2_idx = (direction == 0) ? z_idx : (direction == 1) ? x_idx : y_idx;

        const bool on_face1 =
          ((constraint_mask & face1_type) !=
           dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) ?
            (face1_idx == 0) :
            (face1_idx == fe_degree);
        const bool on_face2 =
          ((constraint_mask & face2_type) !=
           dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) ?
            (face2_idx == 0) :
            (face2_idx == fe_degree);
        return ((((constraint_mask & face1) !=
                  dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) &&
                 on_face1) ||
                (((constraint_mask & face2) !=
                  dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) &&
                 on_face2) ||
                (((constraint_mask & edge) !=
                  dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) &&
                 on_face1 && on_face2));
      }
      ...
      interpolate_boundary_2d(
        const Kokkos::TeamPolicy<MemorySpace::Default::kokkos_space::execution_space>::member_type
                                                                     &team_member,
          ...
          [&](const int &q_point)
            {
              ...
              const bool constrained_dof =
                is_constrained_dof_2d<fe_degree, direction>(constraint_mask, x_idx, y_idx);

              if (constrained_face && constrained_dof)
                {
                  Number     sum = 0.;
                  const bool type =
                    (constraint_mask & this_type) !=
                    dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained;
                  ...
                  if (type)
                    {
                      for (unsigned int i = 0; i <= fe_degree; ++i)
                        {
                          const unsigned int real_idx = (direction == 0) ?
                                                          index2<n_q_points_1d>(i, y_idx) :
                                                          index2<n_q_points_1d>(x_idx, i);

                          const Number w = transpose ?
                      ...
                      for (unsigned int i = 0; i <= fe_degree; ++i)
                        {
                          ...
                          const Number w =
                            transpose ?
                              constraint_weights[(fe_degree - i) * n_q_points_1d + fe_degree -
                                                 interp_idx] :
                              constraint_weights[(fe_degree - interp_idx) * n_q_points_1d +
                                                 fe_degree - i];
                          sum += w * values[real_idx];
                        }
                    }
                             ...
                             [&](const int &q_point)
                               {
                                 ...
                                 const bool         constrained_dof =
                                   is_constrained_dof_2d<fe_degree, direction>(constraint_mask,
                                                                               ...
                                                                               y_idx);
                                 if (constrained_face && constrained_dof)
                                   values[index2<fe_degree + 1>(x_idx, y_idx)] = tmp[q_point];
                               });
        ...
        ViewType                                                     values)
      {
        ...
        const auto this_type =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_x :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_y :
                             dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_z;
        const auto face1_type =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_y :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_z :
                             dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_x;
        const auto face2_type =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_z :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_x :
                             dealii::internal::MatrixFreeFunctions::ConstraintKinds::subcell_y;

        // If computing in x-direction, need to match against face_y or
        // face_z
        const auto face1 =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_y :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_z :
                             dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_x;
        const auto face2 =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_z :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_x :
                             dealii::internal::MatrixFreeFunctions::ConstraintKinds::face_y;
        const auto edge =
          (direction == 0) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::edge_x :
          (direction == 1) ? dealii::internal::MatrixFreeFunctions::ConstraintKinds::edge_y :
          ...
          [&](const int &q_point)
            {
              ...
              const unsigned int interp_idx      = (direction == 0) ? x_idx :
                                                   (direction == 1) ? y_idx :
                                                                      z_idx;
              const bool         constrained_dof = is_constrained_dof_3d<fe_degree, direction>(
                constraint_mask, x_idx, y_idx, z_idx, face1_type, face2_type, face1, face2, edge);
              if ((constrained_face !=
                   dealii::internal::MatrixFreeFunctions::ConstraintKinds::unconstrained) &&
                  constrained_dof)
                {
                  ...
                  if (type)
                  ...
                  else
                    {
                      for (unsigned int i = 0; i <= fe_degree; ++i)
                        {
                          const unsigned int real_idx =
                            (direction == 0) ? index3<n_q_points_1d>(i, y_idx, z_idx) :
                            (direction == 1) ? index3<n_q_points_1d>(x_idx, i, z_idx) :
