...
#ifndef portable_fe_evaluation_h
#define portable_fe_evaluation_h
...
#include "matrix_free_dealii/portable_evaluation_kernels.h"
...
#include "matrix_free_dealii/portable_tensor_product_kernels.h"
...
namespace Portable
{
  ...
  public:
     ...
     */
    static constexpr unsigned int tensor_dofs_per_cell = tensor_dofs_per_component * n_components;
     ...
     */
    DEAL_II_HOST_DEVICE void
    submit_symmetric_gradient(const SymmetricTensor<2, dim, Number> &sym_grad, const int q_point);
    ...
    , cell_id(data->team_member.league_rank())
  {
    ...
    Assert(n_components_ == precomputed_data->n_components,
           ExcMessage(
             ...
             " but the template argument 4 is set to " + Utilities::to_string(n_components_)));
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::get_current_cell_index()
  {
    return cell_id;
  }
  ...
  DEAL_II_HOST_DEVICE const typename FEEvaluation<dim,
                                                  ...
                                                  n_components_,
                                                  Number>::data_type *
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::get_matrix_free_data()
  {
    return data;
  }
      ...
      [&](const int &i)
        {
          for (unsigned int c = 0; c < n_components_; ++c)
            shared_data->values(i, c) =
              src[precomputed_data->local_to_global(i + tensor_dofs_per_component * c, cell_id)];
        });
    ...
    DeviceVector<Number> &dst) const
  {
    ...
    if (precomputed_data->use_coloring)
      {
        Kokkos::parallel_for(Kokkos::TeamThreadRange(data->team_member, tensor_dofs_per_component),
                             [&](const int &i)
                               {
                                 for (unsigned int c = 0; c < n_components_; ++c)
    ...
    const EvaluationFlags::EvaluationFlags integration_flag)
  {
    ...
    if (fe_degree >= 0 && fe_degree + 1 == n_q_points_1d &&
        precomputed_data->element_type == ElementType::tensor_symmetric_collocation)
      {
        internal::FEEvaluationImplCollocation<dim, fe_degree, Number>::integrate(dof_handler_index,
                                                                                 n_components,
                                                                                 integration_flag,
                                                                                 data);
      }
  ...
  DEAL_II_HOST_DEVICE
    typename FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::value_type
    FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::get_dof_value(
      const int dof_index) const
  {
  ...
  {
    AssertIndexRange(q_point, n_q_points);
    Assert(precomputed_data->JxW.size() > 0, ExcMessage("submit_value() requires precomputed JxW"));
    if constexpr (n_components_ == 1)
      {
        shared_data->values(q_point, 0) = value * precomputed_data->JxW(q_point, cell_id);
      }
    else
      {
        for (unsigned int c = 0; c < n_components; ++c)
          shared_data->values(q_point, c) = value[c] * precomputed_data->JxW(q_point, cell_id);
      }
  }
        ...
        for (unsigned int d_1 = 0; d_1 < dim; ++d_1)
          {
            Number tmp = 0.;
            for (unsigned int d_2 = 0; d_2 < dim; ++d_2)
              tmp += precomputed_data->inv_jacobian(q_point, cell_id, d_1, d_2) * gradient[d_2];
            shared_data->gradients(q_point, d_1, 0) = tmp * precomputed_data->JxW(q_point, cell_id);
          }
      }
    else
      {
          ...
          for (unsigned int d_1 = 0; d_1 < dim; ++d_1)
            {
              ...
              for (unsigned int d_2 = 0; d_2 < dim; ++d_2)
                tmp +=
                  precomputed_data->inv_jacobian(q_point, cell_id, d_1, d_2) * gradient[c][d_2];
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::get_symmetric_gradient(
    const int q_point) const
  {
    AssertIndexRange(q_point, n_q_points);
  ...
  template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
  DEAL_II_HOST_DEVICE Number
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::get_divergence(
    const int q_point) const
  {
    AssertIndexRange(q_point, n_q_points);
    Assert(n_components_ == dim,
  ...
  template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
  DEAL_II_HOST_DEVICE void
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::submit_divergence(
    const Number &div_in,
    const int     q_point)
  {
    AssertIndexRange(q_point, n_q_points);
  ...
  template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
  DEAL_II_HOST_DEVICE void
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::submit_symmetric_gradient(
    const SymmetricTensor<2, dim, Number> &sym_grad,
    const int                              q_point)
  {
    AssertIndexRange(q_point, n_q_points);
    Assert(n_components_ == dim,
           ExcMessage("Function submit_symmetric_gradient() only works when "
                      ...
                      "are equal."));
    Assert(precomputed_data->inv_jacobian.size() > 0,
           ExcMessage("submit_symmetric_gradient() requires precomputed inv_jacobian"));
    Assert(precomputed_data->JxW.size() > 0,
      ...
      for (unsigned int d_1 = 0; d_1 < dim; ++d_1)
        {
          Number tmp = 0.;
          for (unsigned int d_2 = 0; d_2 < dim; ++d_2)
            tmp += precomputed_data->inv_jacobian(q_point, cell_id, d_1, d_2) * sym_grad[c][d_2];
          shared_data->gradients(q_point, d_1, c) = tmp * precomputed_data->JxW(q_point, cell_id);
        }
  }
