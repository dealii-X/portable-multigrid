...
#ifndef portable_tensor_product_kernels_h
#define portable_tensor_product_kernels_h
  ...
  namespace Portable
  {
    namespace internal
    {
        ...
        ViewTypeOut                                        out)
      {
        ...
        auto thread_policy =
          Kokkos::TeamThreadMDRange<Kokkos::Rank<2>, TeamType>(team_member, Nj, Nq);
        ...
        ViewTypeOut                                        out)
      {
        ...
        auto thread_policy =
          Kokkos::TeamThreadMDRange<Kokkos::Rank<3>, TeamType>(team_member, Ni, Nj, Nq);
        Kokkos::parallel_for(
          thread_policy,
          [&](const int i, const int j, const int q)
            {
              const int base_shape   = contract_over_rows ? q : q * n_columns;
              const int stride_shape = contract_over_rows ? n_columns : 1;
              ...
              Number sum = shape_data(base_shape) * in(base_in);
              for (int k = 1; k < Nk; ++k)
                sum += shape_data(base_shape + k * stride_shape) * in(base_in + k * stride_in);
                                 ...
                                 for (int k = 1; k < Nk; ++k)
                                   {
                                     ...
                                     sum +=
                                       shape_data(base_shape + k * stride_shape) * in(index_in);
                                   }
      ...
      template <EvaluatorVariant variant,
                ...
                int              n_columns,
                typename Number,
                typename ShapeDataMemorySpace = MemorySpace::Default::kokkos_space>
       ...
       */
      template <int dim, int n_rows, int n_columns, typename Number, typename ShapeDataMemorySpace>
                                                           ...
                                                           ViewTypeOut      out) const
      {
        if constexpr (in_place)
          ...
          apply<dim, n_rows, n_columns, Number, ShapeDataMemorySpace, direction, dof_to_quad, add>(
      ...
      namespace batched
      {
          ...
          const int            batch_size = 1)
        {
          ...
          Assert(in.size() >= static_cast<std::size_t>(batch_size) * n_in_per_elmt,
                 ExcInternalError());
          Assert(out.size() >= static_cast<std::size_t>(batch_size) * n_out_per_elmt,
                 ExcInternalError());

          Kokkos::parallel_for(
            Kokkos::TeamVectorRange(team_member, batch_size * n_blocks1 * n_blocks2),
            [&](const int tid)
              {
                ...
                apply_matrix_vector_product<n_rows,
                  ...
                  Kokkos::subview(out,
                                  Kokkos::make_pair(out_offset, static_cast<int>(out.extent(0)))));
              });
        ...
        public:
           ...
           * above function.
           */
          template <bool transpose, bool add, typename ViewTypeIn, typename ViewTypeOut>
          DEAL_II_HOST_DEVICE void
          co_gradients(const ViewTypeIn in, ViewTypeOut out) const;
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        EvaluatorTensorProduct<evaluate_general, dim, n_rows, n_columns, Number>::values(
          const ViewTypeIn in,
          ViewTypeOut      out) const
        {
          if constexpr (in_place)
            {
              ...
              populate_view<add>(team_member, out, temp, batch_size * n_blocks1 * nn * n_blocks2);
            }
          else
            {
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        EvaluatorTensorProduct<evaluate_general, dim, n_rows, n_columns, Number>::gradients(
          const ViewTypeIn in,
          ViewTypeOut      out) const
        {
          if constexpr (in_place)
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        EvaluatorTensorProduct<evaluate_general, dim, n_rows, n_columns, Number>::co_gradients(
          const ViewTypeIn in,
          ViewTypeOut      out) const
        {
          if constexpr (in_place)
                ...
                for (int n = 0; n < n_columns; ++n)
                  {
                    if constexpr (!transpose)
                      reg[dim - 1][n] =
                        in(elmnt_idx * n_q_points + reminder + n * co_dimension_size);
                    else
                      reg[dim - 1][n] =
